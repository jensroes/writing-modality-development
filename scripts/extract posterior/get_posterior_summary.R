...
model_summary_bfs <- map(seq(length(h)), ~h[[.x]]) %>%
  ...
  separate(DV, into = c("DV", "Pred"), sep = "_" ) %>% 
  select(DV, Pred, Estimate, Q2.5, Q97.5, H0, H1)

model_summary_bfs %>% mutate(across(where(is.numeric), round, 2))

model_summary_bfs %>% filter(Pred == "polytime22:modalitytyping")

# Save model results summary
write_csv(model_summary_bfs, "stanout/posterior_summary.csv")
