...
modality <- readRDS(file = "data/modality_development.Rdata") %>%
  mutate(story_grammar = ordered(story_grammar),
         ...
         cond = paste(time, modality, sep = "_")) 
...
# Formulae
f <- bf(advanced_structures ~ 0 + cond + (time|s|child:school) + (time|p|school), 
        family = negbinomial()) +
     bf(events ~ 0 + cond + (time|s|child:school) + (time|p|school), 
        family = negbinomial()) +
     bf(syntax ~ 0 + cond + (time|s|child:school) + (time|p|school), 
        family = negbinomial()) +
     bf(number_words ~ 0 + cond + (time|s|child:school) + (time|p|school), 
        family = negbinomial()) + 
     bf(story_grammar ~ 0 + cond + (time|s|child:school) + (time|p|school), 
        family = sratio()) +
     bf(vocab_mean_age ~ 0 + cond + (time|s|child:school) + (time|p|school), 
        family = gaussian()) +
     bf(spelling_correct | trials(number_words) ~ 0 + cond + (time|s|child:school) + (time|p|school), 
        family = binomial(link = "logit")) + 
     bf(correct_spaces | trials(spacing_total) ~ 0 + cond + (time|s|child:school) + (time|p|school), 
        family = binomial(link = "logit")) + 
     bf(correct_terminators | trials(terminators_total) ~ 0 + cond + (time|s|child:school) + (time|p|school), 
        family = binomial(link = "logit")) 
