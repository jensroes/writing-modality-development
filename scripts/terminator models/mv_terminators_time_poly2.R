...
# Formulae
f <- bf(additional_terminators ~ poly(time, 2) +  
          (poly(time, 1)|s|child:school) + (poly(time, 1)|p|school), 
        family = negbinomial()) +
     bf(correct_terminators ~ poly(time, 2) +  
          (poly(time, 1)|s|child:school) + (poly(time, 1)|p|school), 
        family = negbinomial()) +
     bf(missing_terminators ~ poly(time, 2) +  
          (poly(time, 1)|s|child:school) + (poly(time, 1)|p|school), 
        family = negbinomial()) 
...
prior <- set_prior("student_t(5, 0, 10)", class = "b", coef = "polytime21", resp = dvs) +
...
# Save model
saveRDS(m, 
        file = "stanout/mv_terminators_time_poly2.rda", 
        compress = "xz")
