...
# Formulae
f <- bf(correct_spaces | trials(spacing_total) ~ poly(time, 2) * modality + (poly(time,1)|s|child:school) + (poly(time,1)|p|school), 
...
# Save model
saveRDS(m, 
        file = "stanout/spacing_time_x_modality_poly2.rda", 
        compress = "xz")
