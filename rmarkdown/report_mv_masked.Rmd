...
knitr::opts_chunk$set(fig.align = 'center', pdf.options(encoding = "ISOLatin9.enc"))  

# To save plots
#knitr::opts_chunk$set(dev = c("pdf", "png"), fig.keep = "all", fig.path = "figures/", fig.align = 'center', pdf.options(encoding = "ISOLatin9.enc"))  
...
The frequency of classroom-activity ratings is shown by mode and for each type of classroom activity in the following figure:

```{r classroomactivity, fig.align='center', fig.cap='Frequency of classroom-activity ratings by mode.', fig.pos='H', fig.width=7.5, fig.height=4}
class_data %>%
...
We tested whether there is a mode difference for different types of class room activities. We used a multivariate model with cumulative distribution for outcome variables `discuss_text`, `discuss_structure`, and `write_story` with one observation each per class room (*N*=`r nrow(class_data)`, i.e. `r nrow(class_data)/2` per mode).  Model comparisons in leave-one-out cross-validation revealed for improvement for the model with mode as fixed effect.
...
The posterior for each outcome variable individually, rather than for class activity overall as shown in the previous model comparison, revealed inconclusive evidence for a mode effect on outcome measures discuss text and discuss structure but moderate evidence for a higher write-story score for keyboard typing compared to handwriting.
...
class_post %>%  
  knitr::kable(align =c("l", "r", "r"), 
               caption = "Mode effect by outcome variable (multivariate model).") %>%
  kable_styling(position = "center", full_width = F) %>%
...
```{r classroomactivity2, fig.align='center', fig.cap='Frequency of classroom-activity ratings by mode.', fig.pos='H', fig.width=7.5, fig.height=6}
#Similar dot plots of six more questions from the questionnaire? These questions concern writing of words, sentences and text digitally and by hand, and they are measured on the same scale as the already included questions about classroom activity. Data can be found in the document “teacher quest.xlsx” (the sheets “describe conditions” and “read me describe conditions”) in dropbox > analysis > data > study2_3. I also attach it here: teacher quest.xlsx
readxl::read_xlsx("../data/teacher_quest.xlsx", sheet = 2) %>%
  ...
  mutate(across(name, recode_factor, 
                T_WJWordPaper = "Write words on paper",
...
```{r}
class_data <- readRDS(file = "../data/modality_development.Rdata") %>%
  select(school, teacher_id, discuss_text, discuss_structure, write_story, modality) %>%
  drop_na() %>% unique()

class_data <- readxl::read_xlsx("../data/teacher_quest.xlsx", sheet = 2) %>%
  rename(modality = condition, teacher_id = teacherID) %>%
  left_join(class_data, by = c("teacher_id", "modality")) %>%
  pivot_longer(-c(teacher_id, modality, school)) %>%
  mutate(name = recode_factor(name, 
                              write_story = "Write story",
                              discuss_text = "Discuss text",
                              discuss_structure = "Discuss structure",
                              T_WJWordPaper = "Write words on paper",
                              T_WJWordNet =	"Write words on tablet",
                              T_TJSimpSent =	"Write sentences on paper",
                              T_TJSentNet =	"Write sentences on tablet",
...
class_data_results <- class_data %>%
  group_by(modality, name) %>%
  summarise(median = median(value),
            IQR = IQR(value)) %>%
  select(name, modality, median, IQR) %>%
  pivot_wider(names_from = modality, values_from = c(median, IQR)) %>%
  select(Measure = name, ends_with("handwriting"), ends_with("typing")) %>%
  unite("Handwriting", median_handwriting:IQR_handwriting, sep = " (") %>%
  unite("Digital", median_typing:IQR_typing, sep = " (") %>%
  mutate(across(c(Handwriting, Digital), paste0, ")"))

write_csv(class_data_results, "tables/classroom_data.csv")
```
...
ps <- posterior_samples(fit, pars = "^b_") %>% 
  ...
  mutate(dv = recode_factor(dv, 
         ...
         across(modality, recode_factor, 
            ...
            typing = "Digital", 
            .ordered = T)) %>%
  rename(`Mode by outcome` = modality)

write_csv(ps, "tables/textfeatures_cellmeans_by_modality_and_time.csv")
...
We incrementally increased model complexity by adding first timepoint (1-5), then mode (levels: handwriting, digital), and their interaction:


1. `y ~ 1 + (time|s|child:school) + (time|p|school)`
2. `y ~ time + (time|s|child:school) + (time|p|school)`
3. `y ~ time + mode + (time|s|child:school) + (time|p|school)`
4. `y ~ time + mode + time:mode + (time|s|child:school) + (time|p|school)`
...
#5. `y ~ time + mode + time:mode + (discuss text + discuss structure + write story) + (time|s|child:school) + (time|p|school)`
#6. `y ~ time + mode + time:mode * (discuss text + discuss structure + write story) + (time|s|child:school) + (time|p|school)`
...
cormat <- read_csv("correlationmatrix.csv") %>%
    mutate(pred = recode(pred, numberwords = "Number of words",
           ...
           across(everything(), str_remove_all, "NA \\[NA -- NA\\]")) %>%
...
write_csv(cormat, "tables/correlationmatrix.csv")
```


## Timecourse function

Time was modeled as first order orthogonal polynomial (linear function) and as second (quadratic) and third order polynomial (cubic). Model comparisons in leave-one-out cross-validation revealed highest predictive performance for the cubic function which, however, was non-different from the quadratic model. Therefore, we use the quadratic function for modelling the time course in the models with mode as factor.
...
write_csv(time, "tables/model_comparison_time_function.csv")
```
...
Highest predictive performance was found to model that included the timecourse-by-mode interaction. In other words, writing mode has a meaningful effect on the timecourse of writing development.
...
post_mod <- read_csv("../stanout/model_comparison_nocovars.csv") %>% 
  mutate(Model = str_remove(string = model, pattern = "mv_"),
         Model = recode(Model, "null" = "Intercept only",
                               ...
                               "time_modality_poly2" = "Mode + Time + Time$^2$",
                               "time_x_modality_poly2" = "Mode $\\times$ (Time + Time$^2$)")) %>%
  select(Model, elpd_diff, se_diff, elpd_loo, se_elpd_loo) %>%
  mutate_if(is.numeric, round, 0) %>%
  unite(col = "diff", c(elpd_diff, se_diff), sep = " (") %>%
  unite(col = "fit", c(elpd_loo, se_elpd_loo), sep = " (") %>%
...
write_csv(post_mod, "tables/model_comparison_timecourse_modality_effect.csv")
```

```{r postmod,  results='asis'}
post_mod %>% knitr::kable(align =c("l", "r", "r"), 
               caption = "Model comparisons for mode timecourse effects. Predictive performance was indicated as expected log pointwise predictive density ($\\widehat{elpd}$). The top row shows the model with the highest predictive performance, i.e. the highest $\\widehat{elpd}$; the differences $\\Delta\\widehat{elpd}$ are relative to the model with the highest predictive performance.") %>%
  kable_styling(position = "center", full_width = F) %>%
  footnote("Standard errors are shown in parentheses.", threeparttable = T)  %>%
...
The following table shows the parameter values estimates for each outcome variable (rows) for main effects of mode and timepoint and their interaction (columns). Evidence for an interaction of mode and the quadratic timepoint term was highlighted in red; evidence in support of the null hypothesis was highlighted in green.
...
post_sum <- read_csv("../stanout/posterior_summary.csv") %>%
    mutate(DV = recode_factor(DV, 
           ...
           Pred = recode_factor(Pred, polytime21 = "Time",
                               polytime22 = "Time$^2$",
                               modalitytyping = "Mode",
                               `polytime21:modalitytyping` = "Mode $\\times$ Time",
                               `polytime22:modalitytyping` = "Mode $\\times$ Time$^2$",
                                .ordered = T)) %>%
    arrange(DV, Pred)
...
post_sum_table <- post_sum %>%
  ...
  select(Outcome, ends_with("_Time"), ends_with("_Time$^2$"), ends_with("_Mode"),
                  ends_with(" Time"), ends_with(" Time$^2$")) 

names(post_sum_table)[-1] <- c("Estimate", "H~1~")

bfs <- filter(post_sum, Pred == "Mode $\\times$ Time$^2$") %>% pull(H1)
...
write_csv(post_sum_table, "tables/model_coefficients_by_outcome_over_time_in_columns.csv")

post_sum_table %>%
  knitr::kable(align =c("l", rep("r",10)), 
               caption = "Model coefficients by outcome variable (multivariate model). Outcomes measures are shown in rows and predictor variables in columns. Moderate to strong evidence in favour of a time-by-mode interaction was highlighted in red; evidence against the alternative hypothesis was highlighted in green.") %>%
  kable_styling(position = "center", full_width = F, font_size = 11) %>%
  footnote("Upper and lower indicate the bounds of 95% PIs. H~1~ indicates the statistical support for the alternative hypothesis over the null corresponding (Bayes Factor).", threeparttable = T)  %>%
  add_header_above(c(" " = 1, "Time" = 2, "Time$^2$" = 2, "Mode" = 2,
                     "Mode $\\times$ Time" = 2,"Mode $\\times$ Time$^2$" = 2)) %>%
...
The overall contribution of adding the by-mode interaction was determined by comparing a a simple main effects models to a model with time-by-mode interactions for each correct spacing and correct spelling. Models were compared using Bayes Factors estimated via bridge sampling [@bennett1976efficient;@meng1996simulating]. We found evidence in favour of the interaction model for both spacing (BF$_{10}$ = `r bf_spacing`) and spelling (BF$_{10}$ = `r bf_spelling`).



## Timecourse visualisation

The posterior of the best fitting model was used to visualise the mode-timecourse effects for each outcome variable. For events, number of words, and syntax we found non-different timecourse effects for handwriting and digital. In other words, development in these skills had the same growth rate. Different growth rates were found for correct spacing, correct spelling, and correct terminators after accounting for longer texts for later timepoints. For correct spacing and correct spelling we observe a learning curve for handwriting that is catching up with the performance of keyboard typists. The pattern observed in the posterior for correct terminators is inconclusive (we will come back to this below). For advanced structures and story grammar we observe growth but no conclusive evidence as to whether the function was different for writing mode. The evidence for timecourse changes and mode-specific effects was negligible for vocabulary mean age.


```{r timecourseposterior, fig.align='center', fig.cap='Posterior of the multivariate analysis. Shown are the timecourse effects for each outcome variable by mode.', fig.pos='H', fig.width=8.5, fig.height=6}

plot_post <- read_csv("../stanout/posterior_for_plotting.csv") %>%
  mutate(across(c(est, lo, up), ~ifelse(dv == "storygrammar", . - 1, 
                                 ifelse(dv == "spellingcorrect", . / number_words, 
                                 ifelse(dv == "correctspaces", . / spacing_total, 
                                 ifelse(dv == "correctterminators", . / terminators_total, .))))),
    across(dv, recode_factor, 
           numberwords = "Number of words",
           correctspaces = "Spacing accuracy",
           correctterminators = "Terminator accuracy",
           spellingcorrect = "Spelling accuracy", 
...
ggplot(plot_post, aes(x = time, y = est, 
                      ...
                      fill = modality,
                      linetype = modality)) +
  geom_ribbon(aes(ymin = lo, ymax = up), colour = NA, alpha =  .2) +
  geom_line(size = .5) +
...
To explore the unclear results for terminator accuracy (evidence for time-by-mode interaction but unclear timecourse effect), we analysed the three variables that were used for terminator accuracy separately in a multivariate model with negative-binomial distribution for correct terminators, missing terminators, and additional terminators. 

### Model comparisons

Model comparisons confirmed an interaction for mode and the quadratic function for time.
...
terminator <- read_csv("../stanout/model_comparisons_terminators.csv") %>% 
  mutate(Model = str_remove(string = model, pattern = "mv_terminators_"),
         Model = recode(Model, "time_x_modality_poly2" = "Mode $\\times$ (Time + Time$^2$)",
                               "time_poly2" = "Time + Time$^2$",
                               "time_modality_poly2" = "Mode + Time + Time$^2$",
                               "time" = "Time", 
                               "null" = "Intercept only")) %>%
...
write_csv(terminator, "tables/model_comparison_terminators.csv")
```
...
The model coefficients show strong evidence for a mode-by-time interaction for correct terminators and moderate evidence for additional terminators and missing terminators.
...
write_csv(post_sum_table, "tables/model_coefficents_by_terminator_measure.csv")

post_sum_table %>%
  knitr::kable(align =c("l", rep("r",10)), 
               caption = "Model coefficients by terminator measure (multivariate model). Outcomes measures are shown in rows and predictor variables in columns. Moderate to strong evidence in favour of a time-by-mode interaction was highlighted in red.") %>%
  kable_styling(position = "center", full_width = F, font_size = 11) %>%
  footnote("Upper and lower indicate the bounds of 95% PIs. H~1~ indicates the statistical support for the alternative hypothesis over the null corresponding (Bayes Factor).", threeparttable = T)  %>%
  add_header_above(c(" " = 1, "Time" = 2, "Time$^2$" = 2, "Mode" = 2,
                     "Mode $\\times$ Time" = 2,"Mode $\\times$ Time$^2$" = 2)) %>%
...
The posterior for each temrinator variable is shown in the following figure. The timecourse functions show different profiles for digital and handwriting with, however, a substantial variability.

```{r timecourseterminator, fig.align='center', fig.cap='Posterior of the multivariate analysis. Shown are the timecourse effects for each terminator variable by mode.', fig.pos='H', fig.width=8.5, fig.height=4}

plot_post <- read_csv("../stanout/posterior_for_plotting_terminators.csv") %>%
  mutate(across(dv, recode_factor, additionalterminators = "Additional terminators",
                                correctterminators = "Correct terminators",
                                missingterminators = "Missing terminators", .ordered = T),
        across(modality, recode_factor, 
           handwriting = "handwriting",
...
The strongest evidence for a mode-by-time interaction was found for the number of correct terminators. As the texts children produced were shorter at earlier timepoints, meaning they produced less terminators overall, we adjusted the analysis of the number of correct terminators for text length (i.e. number of words).


#### Model coefficients

After accounting for the possibility that text-length confounded the results of the analysis of correct terminators, we don't observe evidence for mode-by-time interaction.
...
post_sum <- read_csv("../stanout/posterior_summary_terminators_correct.csv") %>%
           mutate(Pred = recode_factor(Pred, polytime21 = "Time",
                               ...
                               modalitytyping = "Mode",
                               `modalitytyping:number_words` = "Mode $\\times$ Number of words",
                               `polytime21:modalitytyping` = "Mode $\\times$ Time",
                               `polytime22:modalitytyping` = "Mode $\\times$ Time$^2$",
                               `polytime21:number_words` = "Number of words $\\times$ Time",
                               `polytime22:number_words` = "Number of words $\\times$ Time$^2$",
                               `polytime21:modalitytyping:number_words` = "Number of words $\\times$ Mode $\\times$ Time",
                               `polytime22:modalitytyping:number_words` = "Number of words $\\times$ Mode $\\times$ Time$^2$",
...
```{r terminatorcorrect, fig.align='center', fig.cap='Posterior of terminators correct after adjusting for text length. Shown are the timecourse effects for terminators correct by mode.', fig.pos='H', fig.width=6.5, fig.height=4}

read_csv("../stanout/posterior_for_plotting_terminators_correct.csv") %>%
  mutate(across(modality, recode_factor, 
           handwriting = "handwriting",
           typing = "digital", 
           .ordered = T)) %>% 
...
#  scale_x_continuous(limits = c(-5, 30)) +
  labs(colour = "Mode", fill = "Mode", y = "Time points", 
       x = "Value")
...
ICC <- left_join(ICCs, ICCc, by = "outcome") %>%
  arrange(outcome) %>%
  mutate(across(outcome, recode_factor, 
           numberwords = "Number of words",
           correctspaces = "Spacing accuracy",
           correctterminators = "Terminator accuracy",
           spellingcorrect = "Spelling accuracy", 
  ...
  mutate(across(c(Schools, Children), dezero, 2))

write_csv(ICC, "tables/icc.csv")
```
